...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "supplierform")) {
  $updateSQL = sprintf("UPDATE tbsupplier SET fname=%s, lname=%s, email=%s, address=%s, phone=%s, password=%s WHERE username=%s",
...
mysql_select_db($database_bookcon, $bookcon);
$query_mydetails = sprintf("SELECT fname, lname, email, address, phone, username, password, regdate FROM tbsupplier WHERE username = %s", GetSQLValueString($colname_mydetails, "text"));
