...
$query_RequestedItems = sprintf("SELECT tbbook.bookid, tbbook.myid, tbbook.userid, item.name,tbbook.quantity,tbbook.amount, tbbook.`desc`,coalesce(sum(tbbook.quantity*tbbook.amount),0)as total,tbbook.regdate,fname,lname,email,address,phone FROM tbbook,item,myitems,tbuser WHERE tbbook.status='Cancelled' and tbbook.userid=tbuser.username and item.itemid=myitems.itemid and tbbook.myid=myitems.myid and myitems.suppid='".$colname_RequestedItems."' GROUP BY tbbook.bookid ORDER BY tbbook.regdate DESC", GetSQLValueString($colname_RequestedItems, "int"));
  ...
  <?php do { ?>
    <tr>
      ...
      <td><div align="center"><?php echo $row_RequestedItems['address']; ?></div></td>
       ...
       <td><div align="center">Cancelled</div></td>
    </tr>
