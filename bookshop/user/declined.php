...
$query_RequestedItems = sprintf("SELECT tbbook.bookid, tbbook.myid, tbbook.userid, item.name,tbbook.quantity,tbbook.amount, tbbook.`desc`,coalesce(sum(tbbook.quantity*tbbook.amount),0)as total,tbbook.regdate,fname,lname,email,address,phone FROM tbbook,item,myitems,tbsupplier WHERE tbbook.status='Declined' and myitems.suppid=tbsupplier.username and item.itemid=myitems.itemid and tbbook.myid=myitems.myid and tbbook.userid='".$colname_RequestedItems."' GROUP BY tbbook.bookid ORDER BY tbbook.regdate DESC", GetSQLValueString($colname_RequestedItems, "int"));
